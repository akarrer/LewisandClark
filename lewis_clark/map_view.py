...
from lewis_clark import assets, expedition_map
...
class MapView:
    ...
    def __init__(self):
        self.map_mode = "hex"  # "overview" | "region" | "hex"
        ...
        self._map_overlay_us = (
            None  # UI_SCALE used for scaled cartouche / compass cache
        )
    ...
    def _canvas_dims(self) -> tuple[int, int]:
        if self.map_mode == "hex":
            return self.CANVAS_W, self.CANVAS_H
        return expedition_map.canvas_size(self.map_mode)

    def set_map_mode(self, mode: str) -> None:
        if mode not in ("overview", "region", "hex"):
            return
    ...
    def _build_canvas(self, state):
        ...
        water_overlay = pygame.Surface((self.CANVAS_W, self.CANVAS_H), pygame.SRCALPHA)
            ...
            for col in range(assets.HEX_COLS):
                ...
                if proc and terr == "plains":
                    ...
                    for wl in range(-30, 32, 5):
                        wy = cy + wl + int(math.sin((col * 1.3 + row * 0.9) * 0.7) * 5)
                        c_w = (
                            115 + rng.randint(-8, 8),
                            150 + rng.randint(-8, 8),
                            185 + rng.randint(-8, 8),
        ...
        for path, (sh_c, mn_c, hi_c, sh_w, mn_w, hi_w, rname, name_c) in zip(
            assets.RIVER_PATHS, RIVER_STYLES
        ):
        ...
        for wx, wy in ridge_wxy:
            ...
            pygame.draw.line(
                surf, (120, 94, 58), (rx - 40, ry + 14), (rx + 40, ry + 14), 3
            )
            # Three peaks
            for mi, sz in [(-28, 22), (0, 28), (28, 20)]:
                ...
                pygame.draw.polygon(
                    surf,
                    (242, 240, 236),
                    [(mx - 7, ry - sz + 9), (mx, ry - sz - 4), (mx + 7, ry - sz + 9)],
                )
        ...
        for i, wp in enumerate(assets.WAYPOINTS):
            ...
            if not self._waypoint_sprite_blt(surf, wx2, wy2, wp_type):
                ...
                if wp_type in ("fort", "start"):
                    sq = 26
                    pygame.draw.rect(
                        surf, (218, 198, 148), (wx2 - sq, wy2 - sq, sq * 2, sq * 2)
                    )
                    pygame.draw.rect(
                        surf, (68, 48, 18), (wx2 - sq, wy2 - sq, sq * 2, sq * 2), 4
                    )
                    # Corner bastions
                    for bx3, by3 in [
                        (wx2 - sq, wy2 - sq),
                    ...
                    pygame.draw.polygon(
                        surf,
                        (100, 78, 48),
                        [(wx2 - 10, wy2 + 8), (wx2, wy2 - 12), (wx2 + 10, wy2 + 8)],
                    )
                    ...
                    for ang_j in [270, 30, 150]:
                        ...
                        pygame.draw.line(
                            surf, (68, 48, 18), (wx2, wy2), (ex_j, ey_j), 3
                        )
    ...
    def _draw_map_chrome(
        ...
        R: pygame.Rect,
        us: float,
        clip_save: pygame.Rect,
        *,
        hint: str,
    ) -> None:
        """Cartouche, compass, zoom bar, and map frame (all map modes)."""
        ...
        if cart_img is None:
            if not hasattr(self, "_louisiana_cartouche_fallback"):
                ...
                self._louisiana_cartouche_fallback = (
                    render_louisiana_cartouche_surface()
                )
        ...
        pygame.draw.rect(
            surf,
            (200, 172, 100),
            (zi_x, zi_y, int(zi_w * zoom_pct / 100), zi_h),
            border_radius=2,
        )
        pygame.draw.rect(
            surf, (140, 112, 60), (zi_x, zi_y, zi_w, zi_h), 1, border_radius=2
        )
        try:
            lbl = fz.render(hint, True, (42, 32, 14))
            lbl_sh = fz.render(hint, True, (220, 200, 150))
            ly = zi_y - zi_lbl_h
    ...
    def draw(self, surf, state):
        ...
        if self._canvas is None or self._canvas_dirty:
            if self.map_mode == "hex":
                ...
                self._canvas = expedition_map.build_surface(self.map_mode)
                self._canvas_dirty = False
        ...
        vis_w = int(R.w / z)
        vis_h = int(R.h / z)
        self.pan_x = max(0, min(cw - vis_w, self.pan_x))
        self.pan_y = max(0, min(ch - vis_h, self.pan_y))

        src_rect = pygame.Rect(int(self.pan_x), int(self.pan_y), vis_w, vis_h)
        if vis_w > 0 and vis_h > 0:
            try:
                sub = self._canvas.subsurface(src_rect.clip(pygame.Rect(0, 0, cw, ch)))
        ...
        # ── Hex gameplay overlays (only in hex mode) ───────────────────────────
        if self.map_mode != "hex":
            us = float(getattr(assets, "UI_SCALE", 1.0))
            hint = (
                "F1 full trail   F2 corridor   F3 hex play"
                "  |  right-drag pan  scroll zoom  R reset"
            )
            self._draw_map_chrome(surf, R, us, clip_save, hint=hint)
            return
    ...
    def _clamp_pan(self, R: pygame.Rect) -> None:
        ...
        vis_h = int(R.h / self.zoom)
        max_px = max(0, cw - vis_w)
        max_py = max(0, ch - vis_h)
        self.pan_x = max(0, min(max_px, self.pan_x))
        self.pan_y = max(0, min(max_py, self.pan_y))
    ...
    def handle(self, event, state, on_hex_click):
        ...
        if event.type == pygame.MOUSEMOTION:
            ...
            if R.collidepoint(mpos):
                if hex_play:
                    ...
                    if (
                        hc
                        and 0 <= hc[0] < assets.HEX_COLS
                        and 0 <= hc[1] < assets.HEX_ROWS
                    ):
                        self.hover_hex = hc
                if self._drag_start:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and R.collidepoint(mpos):
            ...
            elif event.button == 1 and hex_play:
                hc = self.screen_hex(*mpos)
                if hc:
                    on_hex_click(*hc)
    ...
    def zoom_reset(self):
        ...
        if self.map_mode == "overview":
            ...
            self.pan_y = max(0, (ch - vis_h) * 0.1)
        else:
            self.zoom = 0.38
            sx, sy = self.hex_center(*assets.WP_HEX[0])
            self.pan_x = sx - (R.w / self.zoom) / 2
            self.pan_y = sy - (R.h / self.zoom) / 2
        self._clamp_pan(R)
