...
class ScrollPanel:
    ...
    def draw(self, surf):
        if self._dirty:
            self._render()
        r = self.rect
        sh = alpha_surf(r.w + 4, r.h + 4, (0, 0, 0), 100)
        ...
        edge_h = 6
        top_edge = pygame.Surface((r.w, edge_h), pygame.SRCALPHA)
        ...
        for text, font, colour, y in self._items:
            ...
            if sy > r.y - font.get_height():
                ...
                surf.blit(ts_sh, (r.x + 27, sy + 1))
                ts = font.render(text, True, colour)
                surf.blit(ts, (r.x + 26, sy))
